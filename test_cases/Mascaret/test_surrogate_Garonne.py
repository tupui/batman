...
###
#This script is used to build a PC surrogate with Mascaret on the Garonne case
###

# MascaretStudy Object
Study = MascaretStudy('config_Garonne.json', iprint = 0, working_directory = 'study_PCsurrogate')
...
corners = ([15., 15., 15., 1000], [60., 60., 60, 7000])
...
# Build dictionnairy form of x_learning : x_learning_dico
x_learning_dico = []
...
# Run Mascaret for members in x_learning to compute y_learning
y_learning= []
...
# Create PC surrogate from (x_learning, y_learning) with LS method
PC_LS = SurrogateModel('pc', corners, plabels, strategy='LS', degree=7, distributions=dists_ot)
PC_LS.fit(x_learning,y_learning)

## PC-quad Strategy 
# Create PC surrogate from (x_learning, y_learning) with QUADRATURE method
PC_QUAD = SurrogateModel('pc',corners, plabels, strategy='Quad', degree=7, distributions=dists_ot)
# Get Quadrature points
x_quad = PC_QUAD.predictor.sample
x_quad = np.array(x_quad)
(N_quad,_) = np.shape(x_quad)
# Build dictionnairy form of x_q : x_quad_dico
x_quad_dico = []
...
#Run Mascaret on quadrature points x_quad to compute y_quad
y_quad = []
for k in range(N_quad):
    x = x_quad_dico[k]
    print("Study quad#"+str(k))
    print('x=', x )
    Study.initialize_model()
    Output = Study(x)
    y_quad.append(Output['z'])

# Build PC quad surrogate model        
PC_QUAD.fit(x_quad,y_quad)

## Validation
#Validation sample (x_validation)
N_validation = 10000
x_validation = ot.LHSExperiment(ot.ComposedDistribution(dists_ot),N_validation, True, True).generate() 
x_validation = [list(x_validation[i]) for i in range(N_validation)]
x_validation = np.array(x_validation)

# Build dictionnairy form of x_validation : x_validation_dico
x_validation_dico = []
...
# Run Mascaret for members in x_validation compute y_validation
y_validation= []
...
# Computes Predictcions with Surrogates : x_validation
y_PC_LS, _ = PC_LS(x_validation)
y_PC_QUAD, _ = PC_QUAD(x_validation)
# RMSE and Q2 scores for PC-LS
rmse_pc_ls = np.sqrt(mean_squared_error(y_validation, y_PC_LS))
q2_pc_ls = r2_score(y_validation, y_PC_LS)
# RMSE and Q2 scores for PC-quad
rmse_pc_quad = np.sqrt(mean_squared_error(y_validation, y_PC_QUAD))
q2_pc_quad = r2_score(y_validation, y_PC_QUAD)

## Uncertainty quantifications , Sensitivity analysis
# UQ for PC-LS
Ls_uq = UQ(PC_LS, dists=dists, nsample=10000, plabels = plabels, xlabel='x(m)', flabel='H(m)',xdata=curv_abs, fname='UQ_PC_ls')
Ls_uq.error_propagation()
Ls_sobol = Ls_uq.sobol()
# UQ for PC-quad
Quad_uq = UQ(PC_QUAD, dists=dists, nsample=10000, plabels = plabels, xlabel='x(m)', flabel='H(m)',xdata=curv_abs, fname='UQ_PC_quad')
Quad_uq.error_propagation()
Quad_sobol = Quad_uq.sobol()
# Print  RMSE , Q2 estimation 
print('----ERROR ESTIMATION----')
print('PC_quad :')
print ('RMSE:',rmse_pc_quad,'Q2:', q2_pc_quad)
print('PC_LS :')
print ('RMSE:',rmse_pc_ls,'Q2:', q2_pc_ls)
