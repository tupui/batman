...
print "Write Error_R2.dat"

with open('./Error_R2.dat', 'w') as f:
        ...
        for j in range(ny):
            for i in range(nx):
                ...
                Err_R2 = Err_R2 + (F12[i,j]-F[i,j])**2
                Err_Div = Err_Div + (F12[i,j]-Err_M)**2

                f.writelines("{:.7E}".format(abs(F12[i,j]/Err_M))+"\t ")
                f.writelines("{:.7E}".format(abs(F[i,j]/Err_M))+"\t ")
                f.writelines('\n')

        f.writelines('\n')

print "Lmax(Error), xmax, ymax, imax, jmax :", Err_max, xmax, ymax, imax, jmax
print "R2(Error) :", 1. - (Err_R2 / Err_Div)
