...
The prediction based on surrogate models can be false when multiple physical behaviours are happening in the same data set. In order to separate these behaviours and build adequate surrogate models for each of these, it is possible to use machine learning tools to cluster data and form local models.

This mixture is done in several steps: 

1. Computation of PCA on data greater than scalars to compress information and allow better results (see documentation on POD for more details),
2. Clustering using unsupervised machine learning tools from Scikit-Learn,
3. Formation of local Kriging models : Local Decomposition Method,
...
Scikit-Learn documentation: http://scikit-learn.org/stable/modules/clustering.html#clustering

Unsupervised machine learning tools from Scikit-Learn are used to cluster data set according to parameters chosen by the user,  especially the number of clusters. The objective of these methods is to put a label on each data according to their affiliation to a cluster. To cover most of possible data type, the methods of KMeans (fast computation time and simplest algorithm), GaussianMixture (use of probability instead of metrics and good results) and DBSCAN (density-based algorithm) are proposed here:

**KMeans**

KMeans API: http://scikit-learn.org/stable/modules/generated/sklearn.cluster.KMeans.html#sklearn.cluster.KMeans

KMeans clustering algorithm use iterative refinement to form clusters. It needs an input of the number of clusters and the data set.
...
The clusters are defined by their centroid. With this step, each data point is assigned to its nearest centroid (based on squared Euclidean distance)

2. Centroid update step:

The centroid are then recomputed. To do so, the mean of all data points assigned to that centroid's cluster is taken:

.. math::
	c_i = \frac{1}{S_i}\Sigma_{x_i \in S_i} x_i

The algorithm iterates between steps one and two until it meets the stopping criteria (maximum number of iteration or minimization of distances) where it converge to a result. This result may be a local optimum so it is possible to do more than one computation to have the best result using random initial centroids or other methods.

The method of MiniBatchKMeans can also be used. This method is a variant of KMeans which uses mini batch by picking randomly some samples from the dataset during the first step of the algorithm. It allows for faster convergence time but reduce the quality of results.

MiniBatchKMeans API: http://scikit-learn.org/stable/modules/generated/sklearn.cluster.MiniBatchKMeans.html#sklearn.cluster.MiniBatchKMeans

**DBSCAN**

DBSCAN API: http://scikit-learn.org/stable/modules/generated/sklearn.cluster.DBSCAN.html#sklearn.cluster.DBSCAN

DBSCAN clustering algorithm is a density-based algorithm for clustering purposes. It needs for input the density :math:`\epsilon` and the integer MinPts.

It uses the concept of :math:`\epsilon`-neighbourhood which define the density by taking the data points in a radius :math:`\epsilon` for each data point. Each point can be low-density or high-density depending on the parameter MinPts which represents a minimum number of neighbours and fix the criteria of this classification by comparing it to the :math:`\epsilon`-neighbourhood of each point.

The two value of :math:`\epsilon` and MinPts can then further classify each point:
- Core points which have at least MinPt points in their neighbourhood. These tend to be in the interior of clusters and are the foundation of them.
- Border points who have less than MinPt points in their neighbourhood but are in the :math:`\epsilon`-neighbourhood of a core point.
- Noise point who are neither of the two precedents.

There are then 3 cases of reachability:
- A point directly density-reachable refers to a point recheable in the :math:`\epsilon`-neighbourhood of a core point.
...
1) Computation of Euclidean distances between each point and determination if a point is in the :math:`\epsilon`-neighbourhood of an other.
2) Assignment to a cluster if the point validate the MinPts criteria and expand the cluster by checking if the :math:`\epsilon`-neighbour points also valide the MinPts criteria.

**Gaussian Mixture**

GMM API: http://scikit-learn.org/stable/modules/generated/sklearn.mixture.GaussianMixture.html#sklearn.mixture.GaussianMixture

Gaussian Mixture algorithm uses probabilistic model that assumes all the data points are generated from a mixture of a finite number of Gaussian distributions with unknown parameters.
...
Gaussian Mixture Model (GMM) consists in modeling the dataset with a mixture distribution of multivariate normal distributions g. Each one is associated to a cluster such that probability density function of a point :math:`B_i` is:
...
Then an Expectation Maximization algorithm (EM) is used on :math:`\Phi`. Baye's theorem can express the expectation of the posterior probability :math:`\gamma_k` of belonging to the cluster k (E-step): 
...
Scikit-Learn documentation: http://scikit-learn.org/stable/supervised_learning.html

The labels of cluster and their data point can be used to form local surrogate models for each of these clusters.
Here, the Kriging method detailled before is used.

In order to make data predictions for new samples, supervised machine learning tools from Scikit-Learn are used to affiliate new points to their clusters then use the local models to predict the data for this sample. These classifiers use the previous clusterised data set along with the labels (from unsupervised machine learning) to classify new data points called training set by applying what they learnt with the previously clusterised data set called trained set. To cover most of possible data type, the methods of GaussianNaiveBayes(), SupportVectorMachine(), K-NearestNeighbours() and GaussianProcess are proposed here: 

**Gaussian Naive Bayes**

GaussianNB API: http://scikit-learn.org/stable/modules/generated/sklearn.naive_bayes.GaussianNB.html#sklearn.naive_bayes.GaussianNB

Gaussian Naive Bayes (GNB) is a method that use Bayes' theorem with the "naive" assumption of indepedance between each pair of features.
...
which then with Baye's theorem: 
...
SVC API: http://scikit-learn.org/stable/modules/generated/sklearn.svm.SVC.html#sklearn.svm.SVC

Support Vector Classification (SVC) is a method that constructs a hyper-plane or sot of hyper-planes in a high or infinite dimensional space to do classification. A good separation is achieved by the hyper-plane that has the largest distance to the nearest trained data points of any class. Different Kernel functions can be used for the decision function.
...
KNN API: http://scikit-learn.org/stable/modules/generated/sklearn.neighbors.KNeighborsClassifier.html#sklearn.neighbors.KNeighborsClassifier

K Nearest-Neighbours (KNN) is a classifier that works on the nearest neighbours to a point by computing its distance to every other points. The parameter K is then used to train the training points.
...
These steps are repeated for each training point in order the classify the training set.

**Gaussian Process**

GPC API: http://scikit-learn.org/stable/modules/generated/sklearn.gaussian_process.GaussianProcessClassifier.html#sklearn.gaussian_process.GaussianProcessClassifier

Gaussian Process Classification (GPC) is a classifier based on class probabilities to make predictions. 