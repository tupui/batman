...
The prediction based on surrogate models can be false when multiple physical behaviours are happening in the same data set. In order to separate these behaviours and build adequate surrogate models for each of these, it is possible to use machine learning tools to cluster datas and form local models.

In order to realise that, PCA is computed on the data when they are greater than a scalar (see documentation on POD for more theorical details) to compress information and compute the machine-learning algorithms faster and better. 

Clustering using Unsupervised Machine Learning
.............................

Unsupervised machine learning tools from Scikit-Learn are used to cluster data set according to parameters chosen by the user,  especially the number of clusters. The objective of these methods is to put a label on each data according to their affiliation to a cluster. Several clustering methods are proposed here :
